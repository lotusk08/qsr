...
let prizes;
const DEFAULT_MESS = [
"Có nên trúng giải nhất hay không, đang phân vân quá...",
"Nghe nói phải ăn chay trước một tháng mới có thể trúng giải lớn đó nha!",
"Thật sự muốn giành giải nhất quá !!!",
"Có ai muốn giải nhất không nhỉ?",
"Giải năm cũng tốt mà, miễn sao mình có thể trúng giải là được",
"Chúc mừng tất cả mọi người!",
"Trúng hay không trúng giải cũng không quan trọng, quan trọng là tất cả mọi người ăn uống thật ngon miệng.",
"Năm mới, chúc mọi việc đều thuận lợi.",
"Là người ăn hôi chuyên nghiệp, tôi sẽ xem xét xem có ai giống tôi không",
"Chúc mọi người ngày càng xinh đẹp hơn!",
"Hãy cùng chiến đấu lụm giải nha !!!"
];
...
function showPrizeList(currentPrizeIndex) {
  ...
  if (currentPrize.type === defaultType) {
    currentPrize.count === "nhiều vô số";
  }
  let htmlCode = `<div class="prize-mess">Giải<label id="prizeType" class="prize-shine">${currentPrize.text}</label><label id="prizeText" class="prize-shine">${currentPrize.title}</label>，còn lại<label id="prizeLeft" class="prize-shine">${currentPrize.count}</label>lượt</div><ul class="prize-list">`;
  prizes.forEach(item => {
...
let setPrizeData = (function () {
  return function (currentPrizeIndex, count, isInit) {
    ...
    if (currentPrizeIndex === 0) {
      prizeElement.prizeType.textContent = "quay cho vui";
      prizeElement.prizeText.textContent = "";
      prizeElement.prizeLeft.textContent = "nhiều";
      return;
    }
